...
class smc:
    ...
    def ob(
        ...
        # 创建两个独立的crossed数组，分别用于看涨和看跌OB的检测
        #breaker = np.full(len(ohlc), False, dtype=bool)
        
        # 第一个循环：处理看涨订单区块(Bullish Order Block)
        for i in range(ohlc_len):
            close_index = i

            # Bullish Order Block 的维护
            if len(ob[ob == 1]) > 0:
        ...
        # 第二个循环：处理看跌订单区块(Bearish Order Block)
        for i in range(ohlc_len):
            print('i',i,'--',ob[i])
            close_index = i
            close_price = _close[close_index]

            # Bearish Order Block 的维护
            if len(ob[ob == -1]) > 0:
                for j in range(len(ob) - 1, -1, -1):
                    if ob[j] == -1:
                        currentOB = j
                        print(f"  检查OB at {currentOB}, breaker={breaker[currentOB]}")
                        print(f"  top={top[currentOB]}, bottom={bottom[currentOB]}")
                        print(f"  current high={_high[close_index]}, low={_low[close_index]}")
                        if breaker[currentOB]:
                            #if _low[close_index] < bottom[currentOB]:
                            if _low[close_index] < bottom[currentOB] and mitigated_index[currentOB] == 0:
                                mitigated_index[currentOB] = close_index
                                # 不重置ob[j]，保留-1值
                                print(f"  重置OB: _low[{close_index}]={_low[close_index]} < bottom[{currentOB}]={bottom[currentOB]}")
                            ...
                            print(f"  设置breaker: _high[{close_index}]={_high[close_index]} > top[{currentOB}]={top[currentOB]}")
                            breaker[currentOB] = True
                            mitigated_index[currentOB] = close_index
            ...
            #print('last_btm_indices',last_btm_indices)
            ...
            if last_btm_index is not None:
                ...
                print('close_price',close_price,'swing_btm_price',swing_btm_price,crossed[last_btm_index])
                print(close_price < swing_btm_price , not crossed[last_btm_index],
                 close_index, last_btm_index,ob[close_index - 5:close_index] )
                print("ob数组中的唯一值:", np.unique(ob[~np.isnan(ob)]))
        ...
        pd.DataFrame(ob_series).to_csv('ob.csv')
